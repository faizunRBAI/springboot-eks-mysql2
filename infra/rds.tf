...
resource "aws_security_group" "db" {
  ...
  description = "MySQL access for the EKS workloads of ${local.name}"
  vpc_id      = aws_vpc.this.id

  ingress {
    description = "MySQL from the cluster node security group"
    from_port   = 3306
    to_port     = 3306
    protocol    = "tcp"
...
resource "aws_db_instance" "this" {
  ...
  engine = "mysql"
  # 8.4 is the current MySQL LTS (8.0 reached EOL April 2026).
  # Major.minor version: AWS selects the current patch and applies it during
  # the maintenance window automatically.
  engine_version             = "8.4"
...
output "database_jdbc_url" {
  ...
  value       = "jdbc:mysql://${aws_db_instance.this.address}:${aws_db_instance.this.port}/appdb?sslMode=VERIFY_IDENTITY"
}
